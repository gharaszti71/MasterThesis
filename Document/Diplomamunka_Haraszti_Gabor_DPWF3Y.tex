...
%Margók:
% HK beállításai
%\usepackage[top=25 mm, bottom=40 mm, left=35 mm, right=25 mm]{geometry}
% saját kikalkulált
\hoffset -1in
\voffset -1in
\oddsidemargin 35mm
...
% Elválasztások
\hyphenation{sze-rint mun-ka-te-rü-le-ten szá-mí-tó-gé-pe-ket ke-ret-rend-szer-be meg-fe-le-lõ-ket mû-kö-dés-é-ért gaz-da-gé-pen vesz-nek szoft-ve-rek meg-fe-le-lõ el-ér-he-tõ-ek ké-nyel-me-sen fel-tö-rek-võ al-lo-ká-lunk hasz-ná-lunk A-lap-ve-tõ-en pá-lya-ter-ve-zés szem-pont-já-ból me-nedzs-ment}
...
\begin{document}
	...
	\begin{center}
		...
		\begin{tabular}{ l l r }
			...
			OE-NIK & Hallgató neve & HARASZTI GÁBOR \\
			2019 & Hallgató törzskönyvi száma: & DPWF3Y \\
			% \hline
		\end{tabular}
	...
	Mint látható a megfogalmazásból a szolgáltató robotok definíciója elég tág, ezért érdemes pár egyéb szempont szerint is megvizsgálni, ami a Robotmechanizmusok jegyzet \cite{RobotCsop} szerint a következõ:\\
	...
	Russell Toris és munkatársai Robot Web Tools-al foglalkozó cikke \cite{RWT} a nevezett, Ros\-bridge-re épülõ Javascript keretrendszerrel foglalkozik. A ROSJS mellett ez a másik feltörekvõ, bár kicsit más készletet tartalmazó, Rosbridge-re épülõ keretrendszer. Alapvetõen három részbõl tevõdik össze az RWT, a {\em roslibjs} egy kliens könyvtár, a {\em ros2djs} és a {\em ros3djs} pedig vizualizációs könyvtár. \\
	...
	A diplomamunka feladat által megvalósítandó PoC környezet kiépítéséhez több technológiát is meg kellett vizsgálni, mint lehetséges alternatívákat és a munkához megfelelõket ki kellett választani. A kialakítandó rendszerben használni kell majd saját szervereken futó virtualizációs környezeteket -- leginkább konténer technológiával --,  futtatni kell HTTP szervert, valamint ROS példányokat. A rendszer részei lesznek az Amazon AWS és Microsoft Azure felhõ szolgáltatók által nyújtott virtualizációs és funkcionális szolgáltatásai is. \\
	
	\section{Operációs rendszer}

	A részletes feladatkiírásnak megfelelõen elsõsorban a nyílt forrású operációs rendszereket vettem számításba. A kereskedelmi Windows változatok teljesen kizáródtak a kiválasztásból, mert rajtuk nem fut a ROS \cite{ROS}. Az open source operációs rendszerek közül a ROS által támogatottak tipikusan azok, melyek a Debian\cite{debian} platform csomagkezelõ szolgáltatásait használják. Pillanatnyilag a három támogatott -- Linux alapú, debian rendszerû -- operációs rendszer az Ubuntu Wily (15.10-es verzió), az Ubuntu Xenial (16.04-es verzió) és Debian Jessie (8-as verzió). Ezek már mind elavult verziók, az Ubuntu LTS\footnote{LTS - Long Term Support (hosszútávú támogatás)} a 18.10-es verziónál tart, míg a Debian a 9.4.0 verziónál.\\
	...
	A Docker elõdje a 2008-ban megjelent LXC~\footnote{Linux containers} volt, amely ugyan konténer alapú, de teljes virtuális gépeket kezel. Ezzel szemben a Docker a konténer formátum egységesítése mellett komoly erõfeszítéseket tett az egyetlen program kedvéért létezõ konténerek mûködéséért.\\ 
	...
	A Rapyuta szolgáltatása már eleve azt a réteget valósítja meg, amit létrehozunk lokális és univerzális felhõ szolgáltatásokkal a feladat során. Az alapvetõen robot kontrollerre tervezett ROS-t a Rosbridge segítségével teszi elérhetõvé a IP hálózaton keresztül, valamint a Cloudroid segítségével hoz létre egy menedzselt szolgáltatást.\\
	
	% Kubernetes - Egy mikroservice és Docker mgmt cucc: https://kubernetes.io/
	
	\section{ROS -- Robot Operating System}
	
	% https://hub.packtpub.com/ros-architecture-and-concepts/
	
	A feladathoz felhasznált alap, a ROS \cite{ROS}, melynek szolgáltatásai kívánjuk a TCP/IP hálózaton kerewsztül elérni valamint Internet-en keresztül távolról szolgáltatni. Alapvetõen arra tervezték, hogy robot kontrolleren fusson és helyi, egyedi protokollokon kommunikáljon a többi ROS példánnyal, Node-al.\\
	
	A megnövekedett feladatok és új kihívások miatt egyre sürgetõbbé vált, hogy elérhetõ legyen a megszokott Internetes protokollokon keresztül, funkcionalitása felkerüljön a felhõbe is. Sajnos eredeti tervezése szerint nagyon kötõdik a fizikai robothoz, az alap struktúra az egy robot egy ROS példány, a kontextusfüggõ paraméterek mindenképpen ROS szinten találhatók meg. Itt tudjuk megadni a konkrét robot paramétereit, valamint a környezetének a leírását is. Viszont van számos olyan funkcionalitás a ROS-hoz -- a hozzá kapcsolódó kiterjedt modul könyvtárban --, melyek nem kontextus függõek és így könnyen leválaszthatóak szolgáltatásként.\\
	
	\begin{figure}[ht!]
		\centering
		\includegraphics[width=0.7\linewidth]{ROS}
		\caption[ROS Architektúra]{ROS Architektúra}
		\label{fig:ros}
	\end{figure}
	
	Alapvetõen a ROS egy middleware, mely bizonyos Linux disztribúciókra épül rá. Tartalmaz számos kódkönyvtárat, valamint több szolgáltatást is nyújt. Architektúráját a \ref{fig:ros} ábra szemlélteti. A ROS számítási háló szinten a következõ elemeket tartalmazza:\\
	
	\begin{description}
		\item[Nodes]A csomópont az operációs rendszeren futó processzt reprezentálja, valamennyi egyéb szolgáltatásnak hozzá kell tartoznia egy Node-hoz. Tipikusan egy rendszer több Node-ot tartalmaz, funkcionalitás szerint szétbontva a feladatokat.\vspace{-9pt}
		\item[Master]A Master a Node-ok számára szolgáltat egy regisztrációs szolgáltatást, ahol a Node-ok meg tudják "hirdetni" szolgáltatáskészletüket. Ha a Master nem mûködik a rendszerben, akkor a Node-ok szolgáltatásai sem érhetõk el. Egy elosztott rendszerben elegendõ egy Master egy számítógépen, míg a Node-ok futhatnak több gépen is.\vspace{-9pt}
		\item[Parameter Server] Gyakorlatilag egy Key-Value store, NoSQL adatbázis szerver egy központi helyen. A Node-ok tudják felhasználni a benne tárolt központi paraméterek értékeit a konfigurációjukhoz, mûködésükhöz.\vspace{-9pt}
		\item[Messages]A Node-ok üzenetekkel kommunikálnak egymás között. Az üzenet olyan adatokat tartalmaz, amelyek információkat ad más csomópontoknak. A ROS-nak sokféle üzenete van, és a szokásos üzenettípusok segítségével akár saját típusú üzenetet is létrehozhatunk.\vspace{-9pt}
		\item[Topic](Téma) Minden üzenetnek nevének kell lennie, amelyet a ROS hálózat elindít. Ha egy csomópont adatokat küld, azt mondjuk, hogy a csomópont közzétesz egy üzenetet a Topic-ban. A csomópontok más csomópontok témáit is megkaphatják, egyszerûen feliratkozva a témára. A csomópont elõfizethet egy témára, még akkor is, ha nincsenek olyan más csomópontok, amelyek erre a konkrét témára vonatkoznak, ami lehetõvé teszi számunkra, hogy elválasszuk a termelést /publish/ a fogyasztástól /subscribe/. Ez egy hagyományos, úgynevezett publisher / subscriber modell. Fontos, hogy a topic-ok neve egyedi legyen!\vspace{-9pt}
 		\item[Services](Szolgáltatások) A témák közzétételekor sokfelé küldhetünk üzenetet, több feliratkozónak is, de ha választ is akarunk akkor ezt nem tehetjük meg a témákkal. A szolgáltatások lehetõvé teszik számunkra a csomópontokkal való interakciót. A szolgáltatásoknak egyedi névvel kell rendelkezniük. Ha egy csomópontnak van egy szolgáltatása, az összes csomópont kommunikálhat vele, köszönhetõen a ROS ügyfélkönyvtárainak.\vspace{-9pt}
 		\item[Bags] A táskák egy formátum ROS üzenetek elmentésére és visszajátszására, fontos mechanizmus az adatok tárolására, -- például a szenzora adatok eltárolására a teszteléshez, fejlesztéshez és ezek szükség szerinti visszajátszása --.\vspace{-9pt}
	\end{description}	
	...
	A megvalósítandó rendszer egy PoC, ami nem feltétlenül egy feladatra optimalizált konkrét megvalósítás, sokkal inkább a technológiák kipróbálására szolgáló rendszer, így minden kipróbálandó technológia része lesz a kialakított architektúrának. Alapvetõen célja, hogy a robotikai kutatások, alkalmazások számára ki lehessen választani a legjobb megoldást a rendelkezésre álló eszközkészletbõl. A rendszer által megvalósított példa feladat a pályatervezés, amit a rendszerben különféle pontokra kihelyezve tesztelünk különbözõ szempontokra.\\
	
	\begin{figure}[th!]
		\centering
		\includegraphics[width=0.7\linewidth]{feladat-architektura-ROS}
		\caption[Docker - ROS konfiguráció]{Docker - ROS konfiguráció}
		\label{fig:feladat-architektura-ros}
	\end{figure}
	
	A \ref{fig:feladat-architektura-ros} ábra bemutatja a tervezett architektúra részletet a ROS-os szolgáltatások szempontjából. Az alapja a hardvert szolgáltató infrastruktúra, amit szándékosan nem neveztem el számítógépnek, mert például a felhõ szolgáltatók esetében nem tudom, hogy ez ténylegesen egy fizikai számítógép-e, vagy egy VM\footnote{Virtual Machine - virtuális gép}. Az infrastruktúrális alap fölött helyezkedik el a kiválasztott operációs rendszer, ami tartalmazni fog egy alap ROS telepítést.\\
	
	Az alap ROS-ra azért van szükség, mert ez egy nem elhanyagolható méretû és közösen használt komponens lesz majd minden konténerben és alapszabály, hogy az egyes konkrét image\footnote{A Docker által készített képfájl, mely tartalmazza az alap rendszerhez képest a különbségeket.}-eknek nem szabad túl nagynak lenniük, lehetõleg tényleg csak a specifikus dolgokat tartalmazzák. Természetesen, amennyiben a platform -- felhõ szolgáltatás -- nem képes elõtelepített komponenseket kezelni a Docker szolgáltatáson, akkor minden egyes image-be bele kell tenni a teljes ROS-t is, valamint ha már eleve adott a ROS-os konténer technológia, mint a Rapyuta esetében, akkor nem szükséges a \ref{fig:feladat-architektura-ros} ábra szerinti rendszert felépíteni.\\
	
	Minden kipróbálandó konfiguráció tartalmazni fog egy Rosbridge példányt is -- szintén egy konténerben --, mely Internet képessé teszi a konténerben futó ROS Node által szolgáltatott funkcionalitást -- esetünkben a pályatervezést -- és minden esetben lesz egy lokális Edge\cite{GP_Cloud} gép, mely az összeköttetést biztosítja a helyi rendszerrel, ahogy az a \ref{fig:feladat-architektura-felho} ábrán látható. A laborban természetesen össze lesz kötve egy UR5-ös robottal a rendszer, melyen megtekinthetjük a pályatervezés eredményét, de ez az UR robotokhoz meglévõ emulált környezetben is lehetséges lesz a fejlesztés ideje alatt.\\
	
	\begin{figure}[th!]
		\centering
		\includegraphics[width=0.7\linewidth]{docker-architektura-Felho}
		\caption[Felhõ konfiguráció]{Felhõ konfiguráció}
		\label{fig:feladat-architektura-felho}
	\end{figure}
	
	%https://github.com/cyberdb/Cloudroid
	A rendszerhez kipróbálásra kerül a Cloudroid\cite{Cloudroid} szoftvert, mely összefogja a Rosbridge által kiajánlott Node-ok szolgáltatásait és egységes felületbe fogja össze, valamint menedzsment interfészt biztosít a Rosbridge szolgáltatások felé.\\
	...
	\begin{thebibliography}{99}
		...
		\bibitem{GP_Cloud}Péter Galambos: Cloud-, Fog-, and Mist Computing in Service of Advanced Robot Applications, 2019, pp.111-222.\vspace{-9pt}
